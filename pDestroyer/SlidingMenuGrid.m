...
-(void) buildGrid:(int)cols rows:(int)rows
{
	...
	for (CCMenuItem* item in self.children)
	{
        ...
        int x= self.position.x + col * padding.x + (iPageCount * winSize.width) + padding.x/2;
		int y = winSize.height - 100 - row * padding.y - padding.y/2;
...
-(void) buildGridVertical:(int)cols rows:(int)rows
{
	...
	{
		// Calculate the position of our menu item. 
		item.position = CGPointMake(self.position.x + col * padding.x +40, self.position.y - row * padding.y + (iPageCount * winSize.height)-40);
...
-(BOOL) ccTouchBegan:(UITouch *)touch withEvent:(UIEvent *)event
{
    ...
    if(selectedItem!=nil && selectedItem.isEnabled){
...
{	
	...
	if( bMoving )
	{
		...
		if( iPageCount > 0 && (fMoveDeadZone < abs(fMoveDelta)))
		{
			// Are we going forward or backward?
			bool bForward = (fMoveDelta < 0) ? true : false;
			
			// Do we have a page available?
			if(bForward && (iPageCount>iCurrentPage))
			{
